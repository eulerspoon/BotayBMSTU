...
void App::RunApp() {
	...
	for (int i = 0; i < classChooseDialog.getNumber(); i++) {
		classChooseDialog.getClassVariant(i).setFont(nakula);
	}
			...
			switch (event.type) {
				...
				case sf::Event::MouseButtonPressed:
					if (classChooseDialog.isActive()) {
						// if (classChooseDialog.getCloseButton().isMouseOver(window)) {
						...
						for (int dayIndex = 0; dayIndex < 6; dayIndex++) {
							for (int classIndex = 0; classIndex < 14; classIndex++) {
								if (days[dayIndex][classIndex].isMouseOver(window)) {
									...
									// std::vector<std::wstring> goodClasses = getClasses(dayIndex, classIndex);
									std::vector<std::wstring> goodClasses{L"Пара 1", L"Пара 2", L"Пара 3"};
